...
function buildPopupHtml(room: EscapeRoom): string {
  ...
  return `
    <div style="font-family:Inter,system-ui,sans-serif;min-width:200px;max-width:260px">
      ${room.imatgeUrl
        ? `<img src="${esc(room.imatgeUrl)}" alt="${esc(room.nom)}" onclick="(function(src){var o=document.createElement('div');o.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.88);z-index:99999;display:flex;align-items:center;justify-content:center;cursor:zoom-out';var i=document.createElement('img');i.src=src;i.style.cssText='max-width:90vw;max-height:90vh;border-radius:10px;box-shadow:0 30px 60px rgba(0,0,0,.6)';o.appendChild(i);o.onclick=function(){document.body.removeChild(o)};document.body.appendChild(o);})(this.src)" style="width:100%;height:130px;object-fit:cover;border-radius:8px;margin-bottom:8px;display:block;cursor:zoom-in" onerror="this.style.display='none'" />`
        : ''}
